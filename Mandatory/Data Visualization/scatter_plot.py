...
def show_scatter_plot(df, course1, course2):
    ...
    for house, color in zip(houses, colors):
        ...
        plt.scatter(house_data[course1], house_data[course2], alpha=0.5, color=color, label=house, s=50)
...
def main():
    ...
    filtered_correlations = [(cor, i, j) for (cor, i, j) in zip(correlations, *upper_triangle_indices) if cor >= 0.7]
    ...
    for cor, idx1, idx2 in filtered_correlations:
        course1, course2 = correlation_matrix.columns[idx1], correlation_matrix.columns[idx2]
